...
class QMixNet(nn.Module):
    def __init__(self, args):
        ...
        if args.two_hyper_layers:
            self.hyper_w1 = nn.Sequential(nn.Linear(args.state_shape, args.hyper_hidden_dim),
                                          nn.ReLU(),
                                          nn.Linear(args.hyper_hidden_dim, args.n_agents * args.hyper_hidden_dim))
            ...
            self.hyper_w1 = nn.Linear(args.state_shape, args.n_agents * args.hyper_hidden_dim)
            # 经过hyper_w2得到(经验条数, 1)的矩阵
            self.hyper_w2 = nn.Linear(args.state_shape, args.hyper_hidden_dim * 1)

        # hyper_w1得到的(经验条数，args.hyper_hidden_dim)矩阵需要同样维度的hyper_b1
        self.hyper_b1 = nn.Linear(args.state_shape, args.hyper_hidden_dim)
        # hyper_w2得到的(经验条数，1)的矩阵需要同样维度的hyper_b1
        self.hyper_b2 =nn.Sequential(nn.Linear(args.state_shape, args.hyper_hidden_dim),
                                     nn.ReLU(),
    ...
    def forward(self, q_values, states):  # states的shape为(episode_num, max_episode_len， state_shape)
        ...
        w1 = w1.view(-1, self.args.n_agents, self.args.hyper_hidden_dim)  # (1920, 5, 32)
        b1 = b1.view(-1, 1, self.args.hyper_hidden_dim)  # (1920, 1, 32)
        ...
        w2 = w2.view(-1, self.args.hyper_hidden_dim, 1)  # (1920, 32, 1)
